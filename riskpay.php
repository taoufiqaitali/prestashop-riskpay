...
class RiskPay extends PaymentModule
{
	...
	public function install()
	{
		try {
			...
			$OrderPending->module_name = $this->name;
			$OrderPending->send_email  = false;
			$OrderPending->invoice     = false;
			$OrderPending->color       = 'RoyalBlue';
			$OrderPending->unremovable = true;
			$OrderPending->logable     = false;
			$OrderPending->add();
			Configuration::updateValue('RISKPAY_PENDING', (int)$OrderPending->id);
		} catch (\Exception $e) {
			// no need to do anything, the state already exists
			Configuration::updateValue('RISKPAY_PENDING', _PS_OS_BANKWIRE_);
		}
	...
	public function hookPaymentOptions($params)
	{
		...
		$option->setCallToActionText($this->l('Pay via credit card'));
